...
The QSim library provides a `ModelFactory` factory to facilitate `QuantumSimulationModel` creation for common use cases.
...
The input to all Qsim workflows is a `QuantumSimulationModel`, which can be generated by the `createModel` method from the `ModelFactory` factory. There are a couple of `createModel` overloads.
...
Please refer to this [section](/deploy/developers/implement_qsim_workflow/) for a tutorial on how to create a new QSim workflow plugin.
...
docker build -t combined/focal . --no-cache
docker run -d -P -it --name focalqcor combined/focal
id=$(docker ps -aqf "name=focalqcor") && docker cp $id:/home/dev/qcor/build/qcor-1.0.0.deb . && docker cp $id:/home/dev/xacc/build/xacc-1.0.0.deb .
...
git status
git add -A 
git commit -m "automated ci build of xacc and qcor focal deb packages"
